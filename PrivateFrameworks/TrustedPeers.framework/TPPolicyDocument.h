/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/TrustedPeers.framework/TrustedPeers
 */

@interface TPPolicyDocument : NSObject {
    NSDictionary * _categoriesByView;
    NSDictionary * _introducersByCategory;
    NSArray * _keyViewMapping;
    NSArray * _modelToCategory;
    NSSet * _piggybackViews;
    NSSet * _priorityViews;
    NSData * _protobuf;
    NSDictionary * _redactions;
    NSSet * _userControllableViewList;
    TPPolicyVersion * _version;
}

@property (nonatomic, retain) NSDictionary *categoriesByView;
@property (nonatomic, retain) NSDictionary *introducersByCategory;
@property (nonatomic, retain) NSArray *keyViewMapping;
@property (nonatomic, retain) NSArray *modelToCategory;
@property (nonatomic, retain) NSSet *piggybackViews;
@property (nonatomic, retain) NSSet *priorityViews;
@property (nonatomic, retain) NSData *protobuf;
@property (nonatomic, retain) NSDictionary *redactions;
@property (nonatomic, retain) NSSet *userControllableViewList;
@property (nonatomic, retain) TPPolicyVersion *version;

+ (void)addCategoriesByView:(id)arg1 toPB:(id)arg2;
+ (void)addIntroducersByCategory:(id)arg1 toPB:(id)arg2;
+ (void)addKeyViewMapping:(id)arg1 toPB:(id)arg2;
+ (void)addModelToCategory:(id)arg1 toPB:(id)arg2;
+ (void)addRedactions:(id)arg1 toPB:(id)arg2;
+ (id)categoriesByViewFromPb:(id)arg1;
+ (id)dictionaryOfSetsFromDictionaryOfArrays:(id)arg1;
+ (id)introducersByCategoryFromPb:(id)arg1;
+ (bool)isEqualKeyViewMapping:(id)arg1 other:(id)arg2;
+ (id)modelToCategoryFromPb:(id)arg1;
+ (id)modelToCategoryRulesFromArray:(id)arg1;
+ (id)policyDocWithHash:(id)arg1 data:(id)arg2;
+ (id)policyDocumentWithInternalVersion:(unsigned long long)arg1 modelToCategory:(id)arg2 categoriesByView:(id)arg3 introducersByCategory:(id)arg4 redactions:(id)arg5 keyViewMapping:(id)arg6 userControllableViewList:(id)arg7 piggybackViews:(id)arg8 priorityViews:(id)arg9 hashAlgo:(long long)arg10 error:(id*)arg11;
+ (id)policyDocumentWithVersion:(unsigned long long)arg1 modelToCategory:(id)arg2 categoriesByView:(id)arg3 introducersByCategory:(id)arg4 redactions:(id)arg5 keyViewMapping:(id)arg6 userControllableViewList:(id)arg7 piggybackViews:(id)arg8 priorityViews:(id)arg9 hashAlgo:(long long)arg10 error:(id*)arg11;
+ (id)redactionWithEncrypter:(id)arg1 modelToCategory:(id)arg2 categoriesByView:(id)arg3 introducersByCategory:(id)arg4 keyViewMapping:(id)arg5 error:(id*)arg6;
+ (id)redactionsFromPb:(id)arg1;

- (void).cxx_destruct;
- (id)categoriesByView;
- (id)cloneWithVersionNumber:(unsigned long long)arg1;
- (id)description;
- (id)encodeProtobuf;
- (id)encodeProtobufWithVersion:(unsigned long long)arg1;
- (unsigned long long)hash;
- (id)introducersByCategory;
- (bool)isEqual:(id)arg1;
- (bool)isEqualToPolicyDocument:(id)arg1;
- (id)keyViewMapping;
- (id)modelToCategory;
- (id)piggybackViews;
- (id)policyWithSecrets:(id)arg1 decrypter:(id)arg2 error:(id*)arg3;
- (id)priorityViews;
- (id)protobuf;
- (id)redactions;
- (void)setCategoriesByView:(id)arg1;
- (void)setIntroducersByCategory:(id)arg1;
- (void)setKeyViewMapping:(id)arg1;
- (void)setModelToCategory:(id)arg1;
- (void)setPiggybackViews:(id)arg1;
- (void)setPriorityViews:(id)arg1;
- (void)setProtobuf:(id)arg1;
- (void)setRedactions:(id)arg1;
- (void)setUserControllableViewList:(id)arg1;
- (void)setVersion:(id)arg1;
- (id)userControllableViewList;
- (id)version;

@end
